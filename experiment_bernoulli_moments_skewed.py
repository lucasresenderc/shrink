...
def task(params):
    ...
    estimates += [get_mean(v, np.mean(v),
    ...
    estimates += [get_mean(v, get_mom(v, int(np.ceil(c_eta*np.log(1/delta)))),
                           rho_atm, c_eta*np.log(1/delta), 1) for v in X]
    estimates += [get_mean(v, get_mom(v, int(np.ceil(c_eta*np.log(1/delta)))),
                           rho_win, c_eta*np.log(1/delta), 1) for v in X]
    ...
    estimates += [get_mean(v, get_tm(v, int(np.ceil(c_eta*np.log(1/delta)))),
                           rho_atm, c_eta*np.log(1/delta), 1) for v in X]
    estimates += [get_mean(v, get_tm(v, int(np.ceil(c_eta*np.log(1/delta)))),
                           rho_win, c_eta*np.log(1/delta), 1) for v in X]
...
range_ts = [(0.63, 0.9), (0.755, 0.9), (0.88, 0.9)]
range_distributions = ["St df=1.26 r=0.9",
                       "St df=1.51 r=0.9", "St df=1.76 r=0.9"]
...
df = pd.concat(df)
df.to_parquet("results_bernoulli_moments_skewed.parquet")
